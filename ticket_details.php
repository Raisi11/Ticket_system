<?php
$pageTitle = 'Ticket Details - Waves Support';
...
// Get ticket
$stmt = $conn->prepare("SELECT t.*, c.category_name FROM tickets t LEFT JOIN categories c ON t.category_id = c.id WHERE t.id = ? AND t.user_id = ?");
$stmt->execute([$ticket_id, $user_id]);
$ticket = $stmt->fetch();

if (!$ticket) {
    echo '<div class="container my-4"><div class="alert alert-danger">Ticket not found.</div></div>';
...
if ($ticket['assigned_to']) {
    $staff = getUserById($conn, $ticket['assigned_to']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty(trim($_POST['comment'] ?? ''))) {
    ...
    $stmt = $conn->prepare("INSERT INTO ticket_comments (ticket_id, user_id, comment, comment_type) VALUES (?, ?, ?, 'customer')");
...
<div class="container my-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="my_tickets.php" class="btn btn-secondary"><i class="fas fa-arrow-left me-2"></i>Back</a>
    </div>
                ...
                <div class="card-body">
                    <?php if (count($comments) > 0): ?>
                        <?php foreach ($comments as $c): ?>
                            <div class="mb-3 p-3 rounded <?php echo $c['role'] === 'customer' ? 'bg-light' : 'bg-info bg-opacity-10'; ?>">
        ...
        <div class="col-md-4">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <p><strong>Assigned To:</strong> <?php echo htmlspecialchars($staff_name); ?></p>
